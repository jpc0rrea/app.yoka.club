...
export default function EditDailyRecommendationModal({
  date,
  eventId,
  dailyRecommendationId,
}: EditDailyRecommendationModalProps) {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    try {
      await api.put('/daily-recommendations/edit', {
        date,
      ...
      successToast({
        message: 'recomendação editada com sucesso',
      });
    } catch (error) {
      console.error(error);

      errorToast({
        message: 'ocorreu um erro ao editar a recomendação',
      });
    }
  }
  ...
  async function deleteRecommendation() {
    try {
      await api.delete(`/daily-recommendations/${dailyRecommendationId}`);
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <p className="mb-2 text-sm font-semibold text-gray-900">
          editar recomendação de prática para o dia{' '}
          <strong>
